  ...
  for (let packageType in allPackages) {
    ...
    for (let i = 0; i < packageLinks.length; i++) {
      $(packageLinks[i]).on('click', function () {
        if (emailRequired){
          popup({width:'520px'},"下载" + allPackagesDisplayNames[packageType] + "安装包","<span style='margin-bottom:1rem;display:block;'>请输入您的邮件来接收下载安装包的链接。</span><form style='display:inline-block;' id='download-form'>        <input l placeholder='Email' style='margin-bottom:1rem' id='download-email-input' required pattern=\"[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\">        <button class='btn btn-primary' style='margin-bottom:1rem;' id='email-download'><span style='line-height:1;' class='sub-btn' id='downloadthis'>下载</span><div class='lds-ring sub-load'><div></div><div></div><div></div><div></div></div></button>      </form>");
