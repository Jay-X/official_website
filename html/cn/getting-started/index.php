...
<p>TDengine软件分为服务器、客户端和报警模块三部分，服务器部分taosd目前仅能在Linux系统上安装和运行，客户端和报警模块仅能在Linux或Windows上运行。如果应用需要在Mac上运行，只能使用TDengine的RESTful接口连接服务器。硬件支持X64和ARM的CPU系统。用户可根据需求选择通过<a href="#通过源码安装">源码</a>或者<a href="#通过安装包安装">安装包</a>来安装。</p>
...
报警模块的Windows和Linux安装包如下（请参考<a href="https://github.com/taosdata/TDengine/blob/master/alert/README_cn.md">报警模块的使用方法</a>）：
<ul>
